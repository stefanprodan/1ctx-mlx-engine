...
from the MacBook: the box, the engine (mlx-serve) and 1ctx-mlx-engine itself. Everything
...
| What | Value |
|---|---|
| Version | mlx-serve 26.9.5-pre-release.1 (MLX 0.32.2), installed from the Server page on 2026-09-20 after the cut-over to the new name, binary `~/.1ctx-mlx-engine/engine/versions/v26.9.5-pre-release.1/mlx-serve`. Upgrades, rollbacks and flag changes happen on the Server page (`http://$STAGING_HOST:11235/server`), with `include pre-releases` on |
...
(the config read from `/api/engine` on 2026-09-20; prompt lookup is on, which
renders no flag; the policy is the one the hand-written plist had since
2026-09-07). It is also what to enter on the Server page after a wipe:
